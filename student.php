...
session_start();
    ...
    WHERE student_tbl.entity_id=3 AND student_tbl.stu_status = 'Active'";
            ...
            success: function(response) {
                ...
                if (response.success) {
                    Swal.fire({
                        ...
                        $('#docStudentModal').modal('hide'); // Close the modal
        ...
        success: function(response) {
         ...
         var qrcodeUrl = baseUrl + response.qrcode;
           ...
           // $('#bankLink').attr('href', bankUrl).find('#bankImg').text(response.bank);
            $('#qrcodeLink').attr('href', qrcodeUrl).find('#qrcodeImg').text(response.qrcode);
        },
        error: function(xhr, status, error) {
            // Handle errors here
            console.error('AJAX request failed:', status, error);
